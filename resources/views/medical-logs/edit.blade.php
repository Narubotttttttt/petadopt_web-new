<x-app-layout>
    <div class="w-full max-w-6xl mx-auto py-5 sm:py-8 px-4 sm:px-6 lg:px-8 animate-fade-in space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <a href="{{ route('medical-logs.index') }}" class="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition mb-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
                    <span>Back to Medical Logs</span>
                </a>
                <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit Medical Log</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Update verified shelter vaccinations and routine parasite prevention entries.</p>
            </div>
        ...
        <div 
            x-data="{
                ...
                vaccineName: '{{ old('vaccine_name', $medicalLog->vaccine_name ?? '') }}',
                date: '{{ old('date', $medicalLog->date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}',
                administeredBy: '{{ old('administered_by', $medicalLog->administered_by ?: Auth::user()->name) }}',
        ...
        >
            ...
            <div class="lg:col-span-7 bg-white dark:bg-[#12141C] p-5 sm:p-7 rounded-2xl sm:rounded-3xl shadow-sm dark:shadow-xl dark:shadow-black/40 border border-slate-200/80 dark:border-white/[0.07] space-y-6">
                ...
                <form method="POST" action="{{ route('medical-logs.update', $medicalLog) }}" class="space-y-6" @submit="submitting = true">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="pt-2 flex items-center justify-end gap-3">
                        ...
                        >
                            ...
                            <span x-text="submitting ? 'Saving Entry...' : 'Update Medical Entry'"></span>
                        </button>
                    </div>
                </form>
