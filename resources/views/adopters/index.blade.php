<x-app-layout>
    <div class="w-full py-6 px-4 sm:px-6 lg:px-8 space-y-6" 
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="flex items-center gap-3">
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Adopter Profiles</h1>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs sm:text-sm font-bold bg-[#199CA4]/10 dark:bg-[#199CA4]/20 text-[#199CA4] dark:text-[#41C1CB] border border-[#199CA4]/20 dark:border-[#41C1CB]/30">
                        {{ $totalApprovedAdopters }} Adopters ({{ $totalApprovedApplications }} Pets)
                    </span>
                </div>
                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">Manage approved adopters, shelter safety status, vaccine schedules, and clinical medical logs.</p>
            </div>
            
            {{-- Header Action --}}
            <div class="flex items-center gap-3">
                <a href="{{ route('adoption-applications.index') }}" 
                   class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1e1e1e] text-gray-700 dark:text-gray-200 text-xs sm:text-sm font-bold hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-[#199CA4] dark:hover:text-[#41C1CB] transition shadow-xs">
        ...
        {{-- Filters & Search Row --}}
        <div class="bg-white dark:bg-[#1e1e1e] p-4 rounded-2xl shadow-sm border border-gray-200/80 dark:border-gray-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            {{-- Filter Tabs --}}
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
                <a href="{{ route('adopters.index', array_merge(request()->except('filter', 'page'), ['filter' => 'all'])) }}"
                   class="px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition whitespace-nowrap {{ ($filter ?? 'all') === 'all' ? 'bg-[#199CA4] text-white shadow-xs' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800' }}">
            ...
            <form method="GET" action="{{ route('adopters.index') }}" class="flex items-center gap-2 w-full md:w-auto">
                ...
                <div class="relative flex-1 md:w-80">
                    <input type="text" 
                           ...
                           class="w-full pl-10 pr-8 py-2.5 text-xs sm:text-sm bg-gray-50 dark:bg-[#181818] border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 rounded-xl focus:bg-white dark:focus:bg-[#181818] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition placeholder-gray-400 dark:placeholder-gray-500">
                    ...
                    @if($search)
                        <a href="{{ route('adopters.index', request()->except('search', 'page')) }}" class="absolute right-3 top-3 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xs">
                            &times;
                        </a>
                    @endif
                </div>
        ...
        {{-- Main Adopter Table --}}
        <div class="bg-white dark:bg-[#1e1e1e] rounded-2xl sm:rounded-3xl shadow-sm border border-gray-200/80 dark:border-gray-800 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[950px]">
                    <thead class="bg-gray-50 dark:bg-[#252525] border-b border-gray-200/80 dark:border-gray-800 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        <tr>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse($adopters as $adopter)
                            <tr class="hover:bg-gray-50/70 dark:hover:bg-[#282828] transition-colors">
                                
                                {{-- 1. Adopter Profile --}}
                                <td class="py-4 px-5 align-top">
                                    <div class="flex items-start gap-3.5">
                                        @if($adopter->avatar)
                                            <img src="{{ $adopter->avatar }}" alt="{{ $adopter->applicant_name }}" class="w-11 h-11 rounded-2xl object-cover border border-gray-200 dark:border-gray-700 shadow-xs flex-shrink-0">
                                        @else
                                            @php
                                            ...
                                            @endphp
                                            <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#EAF5F6] to-[#d3eef1] dark:from-gray-800 dark:to-gray-700 text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center font-extrabold text-sm border border-[#199CA4]/20 dark:border-gray-600 flex-shrink-0 shadow-xs">
                                                {{ $adopterInitials }}
                                            </div>
                                        @endif
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center gap-1.5 flex-wrap">
                                                ...
                                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-extrabold font-mono bg-cyan-50 dark:bg-cyan-950/60 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800">
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{{ $adopter->applicant_email }}</div>
                                            <div class="text-xs font-semibold text-gray-600 dark:text-gray-300 mt-0.5">{{ $adopter->applicant_phone }}</div>

                                            @if(!empty($adopter->address))
                                                <div class="text-[11px] text-gray-500 dark:text-gray-400 mt-1 flex items-start gap-1 leading-snug">
                                            ...
                                            @if(!empty($adopter->admin_notes))
                                                <p class="text-[11px] text-gray-600 dark:text-gray-300 bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/60 dark:border-amber-800/60 p-1.5 rounded-lg mt-1.5 italic">
                                    ...
                                    <div class="space-y-3">
                                        @foreach($adopter->applications as $app)
                                            ...
                                            <div class="flex items-center gap-3 min-h-[48px]">
                                                @if($petImg)
                                                    <div class="w-11 h-11 rounded-xl overflow-hidden shrink-0 border border-gray-200 dark:border-gray-700 shadow-2xs relative bg-[#F0FBFB] dark:bg-[#282828]">
                                                        ...
                                                        <div class="hidden w-full h-full bg-[#199CA4]/10 text-[#199CA4] dark:text-[#41C1CB] items-center justify-center font-bold text-sm">
                                                            🐾
                                                        </div>
                                                    </div>
                                                @else
                                                    <div class="w-11 h-11 rounded-xl bg-gray-100 dark:bg-[#282828] text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center font-bold text-sm shrink-0 border border-gray-200 dark:border-gray-700">
                                                        🐾
                                                    </div>
                                                @endif

                                                <div class="min-w-0 flex-1">
                                                    <span class="font-bold text-gray-800 dark:text-white block truncate text-sm leading-tight">{{ $pet && $pet->name ? $pet->name : 'Pet #'.$app->pet_id }}</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400 block truncate mt-0.5 font-medium">{{ $pet ? (ucfirst($pet->type) . ' • ' . $pet->breed) : 'Adopted' }}</span>
                                                </div>

                                                @if($app->contract_pdf_path)
                                                    <a href="{{ route('adoption-applications.contract', $app->id) }}" title="Download Contract" target="_blank"
                                                        class="p-1.5 rounded-lg text-gray-400 hover:text-[#199CA4] dark:hover:text-[#41C1CB] hover:bg-gray-100 dark:hover:bg-[#282828] transition shrink-0">
                                            ...
                                            <div class="flex items-center justify-between gap-2 min-h-[48px]">
                                                @if($latestLog)
                                                    <div class="flex items-center gap-2 min-w-0">
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-bold {{ $latestLog->category === 'vaccination' ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800' : 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800' }}">
                                                            {{ $latestLog->category === 'vaccination' ? 'Vaccine' : 'Deworming' }}
                                                        </span>
                                                        <span class="text-xs text-gray-600 dark:text-gray-300 font-medium truncate">{{ $latestLog->date ? $latestLog->date->format('M d, Y') : '' }}</span>
                                                    </div>
                                                @else
                                                    <span class="text-xs text-gray-400 dark:text-gray-500 italic">No medical logs</span>
                                                @endif
                                                ...
                                                @if($latestVaccine && $latestVaccine->next_due_date)
                                                    ...
                                                    @endphp
                                                    <span class="text-[11px] font-bold px-2 py-0.5 rounded-md flex-shrink-0 {{ $isPastDue ? 'bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800' : ($isSoon ? 'bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800' : 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800') }}">
                                        ...
                                        @foreach($adopter->applications as $app)
                                            ...
                                            <div class="flex items-center justify-end gap-2 min-h-[48px]">
                                                <button type="button" 
                                                    ...
                                                    class="px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-700 text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition shadow-2xs cursor-pointer">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 text-center">
                                    <div class="w-12 h-12 rounded-2xl bg-gray-100 dark:bg-gray-800 text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center text-lg mx-auto mb-3 font-bold">
                                        🐾
                                    </div>
                                    <h3 class="text-sm font-bold text-gray-900 dark:text-white">No Adopter Records Found</h3>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">When adoption applications are approved, the adopters and their pets will appear here.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if($adopters->hasPages())
                <div class="p-4 border-t border-gray-200/80 dark:border-gray-800 bg-gray-50/50 dark:bg-[#252525]">
                    {{ $adopters->links() }}
                </div>
            @endif
        </div>
        ...
        <div x-show="showAddModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-900/70 backdrop-blur-xs" @click="showAddModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-200 dark:border-gray-800">
                    
                    {{-- Modal Header --}}
                    <div class="flex items-center justify-between pb-3.5 mb-4 border-b border-gray-200/80 dark:border-gray-800">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    Pet: <span class="font-bold text-[#199CA4] dark:text-[#41C1CB]" x-text="modalPetName"></span> | Adopter: <span class="font-semibold text-gray-700 dark:text-gray-300" x-text="modalAdopterName"></span>
                                </p>
                            </div>
                        </div>
                        <button type="button" @click="showAddModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-2xl font-light leading-none cursor-pointer">&times;</button>
                    </div>
                    ...
                    <form method="POST" action="{{ route('medical-logs.store') }}" class="space-y-4">
                        ...
                        {{-- Category Selector --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Record Type</label>
                            <div class="grid grid-cols-2 gap-2">
                                <button type="button" 
                                    @click="selectedCategory = 'vaccination'"
                                    :class="selectedCategory === 'vaccination' ? 'bg-[#199CA4] text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
                                    class="py-2.5 rounded-xl font-bold text-xs transition cursor-pointer">
                                    💉 Vaccination
                                </button>
                                <button type="button" 
                                    @click="selectedCategory = 'deworming'"
                                    :class="selectedCategory === 'deworming' ? 'bg-[#199CA4] text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
                                    class="py-2.5 rounded-xl font-bold text-xs transition cursor-pointer">
                                    💊 Deworming
                                </button>
                            </div>
                        ...
                        <div x-show="selectedCategory === 'vaccination'" class="space-y-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Vaccine Name *</label>
                                <input type="text" name="vaccine_name" placeholder="e.g. Anti-Rabies, 5-in-1, DHPP" 
                                    class="w-full px-3.5 py-2 text-xs sm:text-sm bg-gray-50 dark:bg-[#181818] border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-white rounded-xl focus:bg-white dark:focus:bg-[#181818] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition placeholder-gray-400 dark:placeholder-gray-500">
                            </div>
                        </div>

                        <div x-show="selectedCategory === 'deworming'" class="space-y-3.5">
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Deworming Brand / Medicine *</label>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Date Administered *</label>
                                <input type="date" name="date" value="{{ date('Y-m-d') }}" required
                                    class="w-full px-3 py-2 text-xs bg-gray-50 dark:bg-[#181818] border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-white rounded-xl focus:bg-white dark:focus:bg-[#181818] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Next Due Date (Optional)</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Administered By (Staff / Admin)</label>
                        ...
                        {{-- Modal Actions --}}
                        <div class="pt-3 border-t border-gray-200/80 dark:border-gray-800 flex items-center justify-end gap-2.5">
                            <button type="button" @click="showAddModal = false" class="px-4 py-2 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition cursor-pointer">Cancel</button>
        ...
        <div x-show="showHistoryModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-900/70 backdrop-blur-xs" @click="showHistoryModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-800">
                    ...
                    <div class="flex items-center justify-between pb-3.5 mb-4 border-b border-gray-200/80 dark:border-gray-800">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    Pet: <span class="font-bold text-[#199CA4] dark:text-[#41C1CB]" x-text="historyPetName"></span> | Adopter: <span class="font-semibold text-gray-700 dark:text-gray-300" x-text="historyAdopterName"></span>
                                </p>
                            </div>
                        </div>
                        <button type="button" @click="showHistoryModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-2xl font-light leading-none cursor-pointer">&times;</button>
                    </div>
                    ...
                    <div class="max-h-[420px] overflow-y-auto pr-1 space-y-3.5">
                        <template x-if="historyLogs.length === 0">
                            <div class="py-10 text-center">
                                <div class="w-10 h-10 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 flex items-center justify-center text-sm mx-auto mb-2 font-bold">
                                    🩺
                                </div>
                                <p class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">No medical records registered yet</p>
                                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">Click "+ Record" on the table to add the first vaccine or deworming entry.</p>
                            </div>
                        </template>

                        <div class="space-y-3" x-show="historyLogs.length > 0">
                            <template x-for="log in historyLogs" :key="log.id">
                                <div class="p-3.5 rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-[#252525] hover:bg-white dark:hover:bg-gray-800 hover:border-[#199CA4]/30 hover:shadow-sm transition">
                                    <div class="flex items-center justify-between gap-2 mb-1.5">
                                        ...
                                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400" x-text="log.date"></span>
                                    </div>
                                    <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        <span x-show="log.administered_by" class="font-semibold text-gray-700 dark:text-gray-300" x-text="'Administered by: ' + log.administered_by"></span>
                                    ...
                                    <p x-show="log.notes" class="text-xs text-gray-600 dark:text-gray-300 mt-1.5 bg-white dark:bg-[#1e1e1e] p-2 rounded-lg border border-gray-200/80 dark:border-gray-800" x-text="log.notes"></p>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    {{-- Modal Footer --}}
                    <div class="pt-3.5 mt-3.5 border-t border-gray-200/80 dark:border-gray-800 flex justify-end">
                        <button type="button" @click="showHistoryModal = false" class="px-4 py-2 text-xs font-bold text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition cursor-pointer">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <div x-show="showHealthModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-900/70 backdrop-blur-xs" @click="showHealthModal = false"></div>
                ...
                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-800">
                    ...
                    <div class="flex items-center justify-between pb-3.5 mb-4 border-b border-gray-200/80 dark:border-gray-800">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    Pet: <span class="font-bold text-[#199CA4] dark:text-[#41C1CB]" x-text="healthPetName"></span> | Adopter: <span class="font-semibold text-gray-700 dark:text-gray-300" x-text="healthAdopterName"></span>
                                </p>
                            </div>
                        </div>
                        <button type="button" @click="showHealthModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-2xl font-light leading-none cursor-pointer">&times;</button>
                    </div>
                    ...
                    <div class="max-h-[420px] overflow-y-auto pr-1 space-y-3.5">
                        <template x-if="healthLogs.length === 0">
                            <div class="py-10 text-center">
                                ...
                                <p class="text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300">No monthly check-ins submitted yet</p>
                                <p class="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">When the adopter submits monthly photos and health updates via the mobile app, they will appear here.</p>
                            </div>
                        </template>

                        <div class="space-y-3" x-show="healthLogs.length > 0">
                            <template x-for="item in healthLogs" :key="item.id">
                                <div class="p-3.5 rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-50/70 dark:bg-[#252525] hover:bg-white dark:hover:bg-gray-800 hover:border-[#199CA4]/30 hover:shadow-sm transition flex gap-3.5">
                                    <div class="w-20 h-20 rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-700 flex-shrink-0 border border-gray-200 dark:border-gray-700">
                                    ...
                                    <div class="min-w-0 flex-1 space-y-1">
                                        ...
                                        <template x-if="item.notes">
                                            <p class="text-[11px] text-gray-600 dark:text-gray-300 bg-white dark:bg-[#1e1e1e] p-2 rounded-lg border border-gray-200/80 dark:border-gray-800" x-text="item.notes"></p>
                                        </template>
                                    </div>
                                </div>
                            </template>
                    ...
                    {{-- Modal Footer --}}
                    <div class="pt-3.5 mt-3.5 border-t border-gray-200/80 dark:border-gray-800 flex justify-end">
                        <button type="button" @click="showHealthModal = false" class="px-4 py-2 text-xs font-bold text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition cursor-pointer">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <div x-show="showStatusModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-900/70 backdrop-blur-xs" @click="showStatusModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-gray-200 dark:border-gray-800">
                    ...
                    <div class="flex items-center justify-between pb-3.5 mb-4 border-b border-gray-200/80 dark:border-gray-800">
                        ...
                        <button type="button" @click="showStatusModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-2xl font-light leading-none cursor-pointer">&times;</button>
                    </div>
                    ...
                    <form method="POST" :action="'/adopters/' + statusProfileId + '/status'" class="space-y-4">
                        ...
                        {{-- Status Selection --}}
                        <div>
                            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1.5">Adopter Standing</label>
                            <div class="space-y-2">
                                <label class="flex items-center gap-2.5 p-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#181818] hover:bg-gray-50 dark:hover:bg-gray-800/80 cursor-pointer">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-[11px] text-gray-500 dark:text-gray-400">Standard verified adopter in regular standing.</span>
                                    </div>
                                </label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Internal Shelter Remarks (Private)</label>
                            <textarea name="admin_notes" rows="3" x-model="statusAdminNotes" placeholder="Private shelter observations, housing conditions, or warnings..."
                                class="w-full px-3 py-2 text-xs bg-gray-50 dark:bg-[#181818] border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-white rounded-xl focus:bg-white dark:focus:bg-[#181818] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition placeholder-gray-400 dark:placeholder-gray-500"></textarea>
                        </div>

                        {{-- Modal Actions --}}
                        <div class="pt-3 border-t border-gray-200/80 dark:border-gray-800 flex items-center justify-end gap-2.5">
                            <button type="button" @click="showStatusModal = false" class="px-4 py-2 text-xs font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition cursor-pointer">Cancel</button>
