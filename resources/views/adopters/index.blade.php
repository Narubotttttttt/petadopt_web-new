<x-app-layout>
    <div class="w-full py-6 px-4 sm:px-6 lg:px-8 space-y-6" 
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('adoption-applications.index') }}" 
                   class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#111827] text-slate-700 dark:text-slate-200 text-xs sm:text-sm font-bold hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-[#199CA4] dark:hover:text-[#41C1CB] transition shadow-xs">
        ...
        {{-- Filters & Search Row --}}
        <div class="bg-white dark:bg-[#111827] p-4 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
            {{-- Filter Tabs --}}
            <div class="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
                <a href="{{ route('adopters.index', array_merge(request()->except('filter', 'page'), ['filter' => 'all'])) }}"
                   class="px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition whitespace-nowrap {{ ($filter ?? 'all') === 'all' ? 'bg-[#199CA4] text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
            ...
            <form method="GET" action="{{ route('adopters.index') }}" class="flex items-center gap-2 w-full md:w-auto">
                ...
                <div class="relative flex-1 md:w-80">
                    <input type="text" 
                           ...
                           class="w-full pl-10 pr-8 py-2.5 text-xs sm:text-sm bg-slate-50 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl focus:bg-white dark:focus:bg-[#0B0F19] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition placeholder-slate-400 dark:placeholder-slate-500">
        ...
        {{-- Main Adopter Table --}}
        <div class="bg-white dark:bg-[#111827] rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200/80 dark:border-slate-800 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[950px]">
                    <thead class="bg-slate-50 dark:bg-[#0D131F] border-b border-slate-200/80 dark:border-slate-800 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                        <tr>
                            ...
                            <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                                ...
                                <td class="py-4 px-5 align-top">
                                    <div class="flex items-start gap-3.5">
                                        @if($adopter->avatar)
                                            <img src="{{ $adopter->avatar }}" alt="{{ $adopter->applicant_name }}" class="w-11 h-11 rounded-2xl object-cover border border-slate-200 dark:border-slate-700 shadow-xs flex-shrink-0">
                                        @else
                                            @php
                                            ...
                                            @endphp
                                            <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#EAF5F6] to-[#d3eef1] dark:from-slate-800 dark:to-slate-700 text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center font-extrabold text-sm border border-[#199CA4]/20 dark:border-slate-600 flex-shrink-0 shadow-xs">
                                                {{ $adopterInitials }}
                                            </div>
                                        @endif
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            @if(!empty($adopter->admin_notes))
                                                <p class="text-[11px] text-slate-600 dark:text-slate-300 bg-amber-50/70 dark:bg-amber-950/40 border border-amber-200/60 dark:border-amber-800/60 p-1.5 rounded-lg mt-1.5 italic">
                                    ...
                                    <div class="space-y-3">
                                        @foreach($adopter->applications as $app)
                                            ...
                                            <div class="flex items-center gap-3 min-h-[48px]">
                                                @if($petImg)
                                                    <div class="w-11 h-11 rounded-xl overflow-hidden shrink-0 border border-slate-200 dark:border-slate-700 shadow-2xs relative bg-[#F0FBFB] dark:bg-slate-800">
                                                ...
                                                @else
                                                    <div class="w-11 h-11 rounded-xl bg-slate-100 dark:bg-slate-800 text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center font-bold text-sm shrink-0 border border-slate-200 dark:border-slate-700">
                                                        🐾
                                                    </div>
                                                @endif
                                                ...
                                                @if($app->contract_pdf_path)
                                                    <a href="{{ route('adoption-applications.contract', $app->id) }}" title="Download Contract" target="_blank"
                                                        class="p-1.5 rounded-lg text-slate-400 hover:text-[#199CA4] dark:hover:text-[#41C1CB] hover:bg-slate-100 dark:hover:bg-slate-800 transition shrink-0">
                                        ...
                                        @foreach($adopter->applications as $app)
                                            ...
                                            <div class="flex items-center justify-end gap-2 min-h-[48px]">
                                                <button type="button" 
                                                    ...
                                                    class="px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition shadow-2xs cursor-pointer">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 text-center">
                                    <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 text-[#199CA4] dark:text-[#41C1CB] flex items-center justify-center text-lg mx-auto mb-3 font-bold">
                                        🐾
                                    </div>
            ...
            @if($adopters->hasPages())
                <div class="p-4 border-t border-slate-200/80 dark:border-slate-800 bg-slate-50/50 dark:bg-[#0D131F]">
                    {{ $adopters->links() }}
                </div>
            @endif
        </div>
        ...
        <div x-show="showAddModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-slate-900/70 backdrop-blur-xs" @click="showAddModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#111827] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-200 dark:border-slate-800">
                    
                    {{-- Modal Header --}}
                    <div class="flex items-center justify-between pb-3.5 mb-4 border-b border-slate-200/80 dark:border-slate-800">
                    ...
                    <form method="POST" action="{{ route('medical-logs.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <button type="button" 
                                    ...
                                    :class="selectedCategory === 'vaccination' ? 'bg-[#199CA4] text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'"
                                    class="py-2.5 rounded-xl font-bold text-xs transition cursor-pointer">
                                    💉 Vaccination
                                </button>
                                <button type="button" 
                                    @click="selectedCategory = 'deworming'"
                                    :class="selectedCategory === 'deworming' ? 'bg-[#199CA4] text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'"
                                    class="py-2.5 rounded-xl font-bold text-xs transition cursor-pointer">
                                    💊 Deworming
                                </button>
                            </div>
                        ...
                        <div x-show="selectedCategory === 'vaccination'" class="space-y-3.5">
                            <div>
                                ...
                                <input type="text" name="vaccine_name" placeholder="e.g. Anti-Rabies, 5-in-1, DHPP" 
                                    class="w-full px-3.5 py-2 text-xs sm:text-sm bg-slate-50 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl focus:bg-white dark:focus:bg-[#0B0F19] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition placeholder-slate-400 dark:placeholder-slate-500">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="date" name="date" value="{{ date('Y-m-d') }}" required
                                    class="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-[#0B0F19] border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl focus:bg-white dark:focus:bg-[#0B0F19] focus:border-[#199CA4] focus:ring-2 focus:ring-[#199CA4]/20 transition">
                            </div>
                            <div>
                        ...
                        {{-- Modal Actions --}}
                        <div class="pt-3 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-end gap-2.5">
                            <button type="button" @click="showAddModal = false" class="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition cursor-pointer">Cancel</button>
        ...
        <div x-show="showHistoryModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-slate-900/70 backdrop-blur-xs" @click="showHistoryModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#111827] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-slate-200 dark:border-slate-800">
                    ...
                    <div class="max-h-[420px] overflow-y-auto pr-1 space-y-3.5">
                        <template x-if="historyLogs.length === 0">
                            <div class="py-10 text-center">
                                <div class="w-10 h-10 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 flex items-center justify-center text-sm mx-auto mb-2 font-bold">
                                    🩺
                                </div>
                        ...
                        <div class="space-y-3" x-show="historyLogs.length > 0">
                            <template x-for="log in historyLogs" :key="log.id">
                                <div class="p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-[#0D131F] hover:bg-white dark:hover:bg-slate-800/80 hover:border-[#199CA4]/30 hover:shadow-sm transition">
                                    ...
                                    <p x-show="log.notes" class="text-xs text-slate-600 dark:text-slate-300 mt-1.5 bg-white dark:bg-[#111827] p-2 rounded-lg border border-slate-200/80 dark:border-slate-800" x-text="log.notes"></p>
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    {{-- Modal Footer --}}
                    <div class="pt-3.5 mt-3.5 border-t border-slate-200/80 dark:border-slate-800 flex justify-end">
                        <button type="button" @click="showHistoryModal = false" class="px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition cursor-pointer">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <div x-show="showHealthModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-slate-900/70 backdrop-blur-xs" @click="showHealthModal = false"></div>
                ...
                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#111827] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-slate-200 dark:border-slate-800">
                    ...
                    <div class="max-h-[420px] overflow-y-auto pr-1 space-y-3.5">
                        ...
                        <div class="space-y-3" x-show="healthLogs.length > 0">
                            <template x-for="item in healthLogs" :key="item.id">
                                <div class="p-3.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-[#0D131F] hover:bg-white dark:hover:bg-slate-800/80 hover:border-[#199CA4]/30 hover:shadow-sm transition flex gap-3.5">
                                    <div class="w-20 h-20 rounded-xl overflow-hidden bg-slate-200 dark:bg-slate-700 flex-shrink-0 border border-slate-200 dark:border-slate-700">
                                    ...
                                    <div class="min-w-0 flex-1 space-y-1">
                                        ...
                                        <template x-if="item.notes">
                                            <p class="text-[11px] text-slate-600 dark:text-slate-300 bg-white dark:bg-[#111827] p-2 rounded-lg border border-slate-200/80 dark:border-slate-800" x-text="item.notes"></p>
                                        </template>
                                    </div>
                                </div>
                            </template>
                    ...
                    {{-- Modal Footer --}}
                    <div class="pt-3.5 mt-3.5 border-t border-slate-200/80 dark:border-slate-800 flex justify-end">
                        <button type="button" @click="showHealthModal = false" class="px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition cursor-pointer">
                            Close
                        </button>
                    </div>
                </div>
        ...
        <div x-show="showStatusModal" x-cloak style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-slate-900/70 backdrop-blur-xs" @click="showStatusModal = false"></div>

                <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

                <div class="inline-block px-5 sm:px-6 pt-5 pb-6 overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#111827] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-slate-200 dark:border-slate-800">
                    ...
                    <form method="POST" :action="'/adopters/' + statusProfileId + '/status'" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                <label class="flex items-center gap-2.5 p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0B0F19] hover:bg-slate-50 dark:hover:bg-slate-800/80 cursor-pointer">
                        ...
                        {{-- Modal Actions --}}
                        <div class="pt-3 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-end gap-2.5">
                            <button type="button" @click="showStatusModal = false" class="px-4 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition cursor-pointer">Cancel</button>
